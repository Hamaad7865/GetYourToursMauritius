...
--
-- `on conflict on constraint activity_content_defaults_pkey` (rather than `on conflict (category)`):
-- 20260901000700 (appended near the end of this file) re-keys that primary key from (category) to
-- (category, locale) but KEEPS THE SAME CONSTRAINT NAME, so targeting it by name resolves correctly
-- whichever shape it currently has — a fresh top-to-bottom run of this file hits it here while it is
-- still (category) alone, while re-running this whole file against an ALREADY-migrated database (this
-- table's normal state) hits it after it has become (category, locale). A literal `(category)` target
-- would raise "no unique or exclusion constraint matching the ON CONFLICT specification" in the second
-- case — see tests/integration/catch-up-parity.test.ts, which applies migrations then layers this
-- whole file on top and caught exactly that.
...
on conflict on constraint activity_content_defaults_pkey do nothing;
...
-- ============================================================================
-- WhatsApp owner alerts (migration 20260817000000). The owner is onboarding a real
-- WhatsApp Cloud API number (webhook shipped at /api/v1/webhooks/whatsapp), so the
-- OWNER chat alert now ALSO goes over WhatsApp — Telegram/email/bell unchanged.
-- Re-applies the winning enqueue_booking_notification VERBATIM (from the Telegram
-- section above) and ONLY adds the two owner WHATSAPP rows (idempotency suffix _wa).
-- 'whatsapp' is an original notification_channel value — no ADD VALUE needed.
-- ============================================================================
create or replace function enqueue_booking_notification()
returns trigger
language plpgsql
...
-- Re-assert 20260814000000's execute lockdown (create-or-replace keeps ACLs, but keep the intent
...
-- ============================================================================
-- Weather disruption + rescheduling (migration 20260819000000). /refunds already
-- PROMISES the guest a choice of full refund or free reschedule when we call a trip
-- off; nothing in the app could deliver it. Adds bookings.disruption, the
-- api_reschedule_booking / api_weather_cancel_occurrence / api_admin_calendar_month
-- RPCs, and re-applies api_cancel_booking + booking_json from their WINNING bodies
-- VERBATIM plus the disruption bypass (and the customer cancellation email that
-- branch never sent). Keep identical to the migration file.
-- ============================================================================
...
create index if not exists guest_reviews_status_idx on guest_reviews (status, submitted_at);
...
-- ---- 20260823000000_review_invite_context -------------------------------------
...
-- ---- 20260824000000_belle_mare_activity_grouping --------------------------------
...
-- ---- 20260825000000_review_invites_completed_status -------------------------------------
...
-- "the trip has definitely happened". Full body carried forward verbatim, only the status filter changed.
create or replace function api_enqueue_review_invites()
returns int
language plpgsql
volatile
...
-- ---- 20260826000000_owner_alert_customer_phone -------------------------------------------
...
-- ===========================================================================================
-- 20260827000000_payment_checkout_liveness.sql
-- ===========================================================================================
...
-- ---- 20260828000000_review_invites_p_arg -------------------------------------------------
-- api_enqueue_review_invites was defined niladic above (20260822 / 20260825), but the shared RPC
-- adapter (src/lib/supabase/rpc.ts) invokes every api_* function as fn(p := <jsonb>). PostgREST looked
-- for a (p) overload that did not exist, so the maintenance cron's review-invite step 503'd every tick
-- since guest reviews shipped. Give it the conventional (p jsonb default '{}') signature — body ignores
-- p — and drop the niladic overload so a bare zero-arg call is never ambiguous. Body VERBATIM from the
-- 20260825 block above; only the signature changes.
...
-- ---- 20260901000000_activity_translation_source -------------------------------------------
...
-- ---- 20260901000100_localised_catalogue_rpcs ----------------------------------------------

-- Locale-aware api_get_activity: left-join activity_translations and coalesce PER FIELD (never per
-- row), so a half-translated activity shows French where it exists and English everywhere else. Array
-- fields use nullif(..., '{}') because these columns are `not null default '{}'` — an empty array
...
-- Locale-aware api_search_activities: the SAME left-join + per-field coalesce as api_get_activity
-- above, extended to the search/cards/rails RPC — this is what actually puts French on every
-- surface a visitor sees BEFORE opening an activity (cards, search results, home rails, related
-- tours, wishlist). TourSummary only carries two translatable fields (title, summary), so those
-- are the only two coalesced; faceting, prices, ratings, images, ordering, pagination, the total
-- — is carried over verbatim from the winning body. The join lives in the `filtered` CTE so it
-- flows unchanged into `priced` (and therefore `total`) and `paged`: activity_translations is
-- unique on (activity_id, locale), so a left join on that key cannot fan out rows or inflate the
-- count.
...
-- ---- 20260901000200_reclassify_stub_translations ------------------------------------------
...
-- ==================== 20260901000400_seed_fr_catalogue.sql ====================
...
-- ==================== 20260901000600_search_activities_locale.sql ====================
...
-- ==================== 20260901000700_translate_content_defaults_and_extra.sql ====================
...
-- ── 20260901000800: dedup the payment ledger by event type ──────────────────────────────────────
...
-- ── 20260901000900: lock operators + revoke payment delete ──────────────────────────────────────
