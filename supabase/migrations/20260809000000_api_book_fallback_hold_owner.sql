-- 20260809000000_api_book_fallback_hold_owner
-- Fix: api_book's FALLBACK hold (minted when no reusable holdId is supplied) landed ownerless.
-- create_hold runs under service_role (api_book is server-only), so it wrote created_by = NULL even
-- though the booking's user_id is set correctly from actorUserId. The customer's owner-scoped hold
-- status/release endpoints then couldn't act on that hold. Stamp created_by = the actor after the
-- booking-owner linkage (reused holds already own themselves; guest bookings stay ownerless).
