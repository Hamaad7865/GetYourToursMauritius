-- api_convert_quote learns the per-line TRANSPORT ADD-ON (20260924000000).
--
-- Re-applies the WINNING body of api_convert_quote (20260912000000) VERBATIM apart from three changes,
-- all about the transport add-on now living on the line:
--
--   1. THE TOTAL CHECK. `quote_total_mismatch` compared quotes.total_minor against Σ subtotal_minor. A
--      line's transport fare is a SEPARATE amount on the row (it is charged on top of the line price and
--      quoteTotalMinor adds it), so the sum here must include Σ transport_fare_minor or a quote with any
--      attached transfer would fail to convert ("total 61600 vs lines 52000").
--   2. THE CUSTOM-LINE COPY. booking_custom_items now carries the three transport columns, copied across.
--   3. THE CATALOGUE-LINE COPY. booking_items now carries them too, so a tour's transfer reaches the day
--      sheet and the receipt.
--
-- Everything else is byte-identical, including the two contracts resolved-function-bodies.test.ts pins:
-- the owner resolve (`quote_owner_for_email`) and the deposit sizing
-- (`v_deposit_minor := round(v_quote.total_minor * v_quote.deposit_bps / 10000…)`). Mirror into
-- supabase/catch-up.sql and regenerate supabase/setup.sql.
