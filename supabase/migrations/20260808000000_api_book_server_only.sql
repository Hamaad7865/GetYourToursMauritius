-- 20260808000000_api_book_server_only
-- Close the authenticated-user rate-limit bypass: api_book was callable by `authenticated`, so a
-- registered user could hit PostgREST directly and skip the route's 15/IP booking limiter. Re-apply
-- api_book replacing auth.uid() with a trusted p.actorUserId (the booking route now calls it via a
-- service-role client, passing the verified user id -- same pattern as the hold lockdown), then
-- revoke it from public/anon/authenticated and grant service_role only.
