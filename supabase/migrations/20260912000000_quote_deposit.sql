...
-- ---------------------------------------------------------------------------------------------------
-- Task 4 — the accountless quote guest's way into the BALANCE checkout.
--
-- create_payment (re-applied above) now carries a 'balance' branch: on a CONFIRMED booking that still
-- owes money it mints — or, under the booking-row lease, REUSES — a separate purpose='balance' payments
-- row sized to the booking's balance_due_minor (a server figure, never caller input), so one balance is
-- one payable Peach session and charging it drives the amount owed to 0.
--
-- api_create_quote_payment was defined in 20260911000000 for the DEPOSIT only (it refused every purpose
-- but 'booking'). This re-applies it as the winning body, widened so the same accountless guest who paid
-- the deposit through the emailed link can pay the balance the same way — the balance link staff send is
-- the same token-authorized entry point, one purpose along. Everything that made the deposit entry sound
-- is UNCHANGED and re-applied verbatim:
--
--   * SERVICE-ROLE ONLY. `revoke ... from public, anon, authenticated` (all three roles named — `from
--     public` alone leaves Supabase's stock direct grants, the omission that has shipped a live hole
--     from this repo three times), because authorization for a quote is the LINK TOKEN verified by
--     resolveQuoteForToken in the route, not the session this function deliberately skips;
--   * narrowed to the quote path — the booking must be one a `quotes` row points at, with source =
--     'quote' — so the identity bypass can never be aimed at an ordinary customer's booking, balance or
--     deposit; and
--   * it still reaches the SHARED create_payment body (`return create_payment(p, false)`), so the quote
--     path and the customer path contend for the SAME single-flight lease and cannot fork a session.
--
-- The pickup add-on is STILL refused here: it is a supplement on a CONFIRMED booking, paid from the
-- booking page by its signed-in owner — a quote guest has no such page and no account, so this entry
-- point should never open one. Only the deposit and the balance are the accountless guest's to pay.
-- ---------------------------------------------------------------------------------------------------
