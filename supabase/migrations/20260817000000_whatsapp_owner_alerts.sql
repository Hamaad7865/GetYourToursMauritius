-- WhatsApp owner alerts (the revival). The owner is onboarding a real WhatsApp Cloud API number
-- (webhook endpoint shipped app-side as /api/v1/webhooks/whatsapp), so the OWNER chat alert on a
-- new/paid-but-unstandable booking now ALSO goes over WhatsApp — Telegram, email and the admin bell
-- are unchanged (redundancy is deliberate: a rare duplicate beats a missed booking alert).
--
-- Drift rule (see 20260810000001): this re-applies the WINNING enqueue_booking_notification body
-- VERBATIM (from 20260810000001_telegram_owner_alerts) and ONLY adds the two owner WHATSAPP rows,
-- with a distinct idempotency suffix _wa. The recipient stays the 'owner' sentinel — the drain
-- resolves OWNER_WHATSAPP_TO at send time (failing loudly when unset), so no phone number is stored
-- in the DB. The 'whatsapp' channel is an original notification_channel value (20260615120000).
