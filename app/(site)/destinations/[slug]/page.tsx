...
export default async function DestinationDetailPage({
  ...
  return (
    <>
      ...
      <InfoPage eyebrow={`Destination guide · ${a.region} coast`} title={`${a.name}, Mauritius`} intro={a.intro}>
        {/* Breadcrumb */}
        ...
        {a.goodFor.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {a.goodFor.map((g) => (
              <span key={g} className="rounded-full bg-teal-tint px-3 py-1 text-[12.5px] font-semibold text-teal-dark">
                {g}
              </span>
            ))}
          </div>
        ...
        <section className="mt-9 border-t border-ink/10 pt-8">
          <h2 className="text-[22px] font-extrabold tracking-tight text-ink">Things to do in {a.name}</h2>
          <List items={a.highlights} />
          <Link href="/attractions" className="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-teal hover:text-teal-dark">
        ...
        {a.beaches.length > 0 && (
          <section className="mt-9 border-t border-ink/10 pt-8">
            <h2 className="text-[22px] font-extrabold tracking-tight text-ink">Beaches near {a.name}</h2>
            <div className="mt-4 flex flex-wrap gap-2.5">
              {a.beaches.map((b) => (
                <span key={b} className="rounded-full border border-ink/12 bg-white px-3.5 py-1.5 text-[13.5px] text-ink/80">
                  {b}
                </span>
              ))}
            </div>
        ...
        {/* Getting there */}
        <section className="mt-9 border-t border-ink/10 pt-8">
          <h2 className="text-[22px] font-extrabold tracking-tight text-ink">Getting to {a.name}</h2>
          <p className="mt-4 text-[15px] leading-relaxed text-ink/80">{a.gettingThere}</p>
          <Link href="/airport-transfers" className="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-teal hover:text-teal-dark">
        ...
        {a.nearbyAttractions.length > 0 && (
          <section className="mt-9 border-t border-ink/10 pt-8">
            <h2 className="text-[22px] font-extrabold tracking-tight text-ink">What&apos;s nearby</h2>
            <div className="mt-4 flex flex-wrap gap-2.5">
              {a.nearbyAttractions.map((n) => (
                <span key={n} className="rounded-full border border-ink/12 bg-white px-3.5 py-1.5 text-[13.5px] text-ink/80">
                  {n}
                </span>
              ))}
            </div>
        ...
        {/* CTA */}
        <section className="mt-9 border-t border-ink/10 pt-8">
          <h2 className="text-[22px] font-extrabold tracking-tight text-ink">Explore from {a.name}</h2>
          <p className="mt-2 max-w-2xl text-[15px] leading-relaxed text-ink/75">
            Book a private day tour or excursion, or design your own day around {a.name} with our free AI
            road-trip planner — with door-to-door pickup and instant confirmation.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <Link href="/ai-road-trip-planner" className="inline-flex items-center gap-2 rounded-full border border-ink/15 px-5 py-2.5 text-sm font-bold text-ink hover:border-teal hover:text-teal">
        ...
        <EnquireRow message={`Hi Belle Mare Tours! I'm staying in ${a.name} and would like to plan some tours and a transfer.`} />
      </InfoPage>
    </>
  );
}
