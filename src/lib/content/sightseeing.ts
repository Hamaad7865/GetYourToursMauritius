 ...
 * an identical, premium set of highlights and know-before-you-go notes regardless of what each tour's
 * admin record happens to carry. The per-tour itinerary + full description still convey the specific
 * places each tour visits — these are the operator promises common to all of them. A "private
 * sightseeing tour" is a vehicle-priced activity (`pricingMode === 'vehicle'`).
 *
 * Social proof used to live here too (a sightseeing-only review/rating fallback). It now applies to
 * every activity and is relevance-matched, so it moved to `activity-reviews.ts` (rating, client-safe)
 * and `activity-reviews-pool.ts` (the reviews themselves, server-only).
 */
