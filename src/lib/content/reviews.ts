...
const TRANSFER_RE =
  /\b(airport|transfer(?:red|s)?|pick(?:ed|s)?[- ]?up|pickup|driver|chauffeur|taxi)\b/i;
export function transferReviews(n: number): FeaturedReview[] {
  const matched = FEATURED_REVIEWS.filter(
    (r) => TRANSFER_RE.test(r.text) || (r.title ? TRANSFER_RE.test(r.title) : false),
  );
