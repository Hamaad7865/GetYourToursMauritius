...
import { countRowsEq, isForeignKeyViolation } from '@/lib/admin/delete-guards';
...
export async function deleteRentalVehicle(slug: string): Promise<void> {
  ...
  // must keep naming the vehicle it was sold as. Postgres says that with a bare constraint name,
  // which reached the fleet screen verbatim — a string the operator cannot act on. Translate it into
  // the decision they actually have, the way src/lib/services/db-errors.ts translates the booking
  // path's constraint failures. (Draft quote lines never land here: their FK is ON DELETE SET NULL.)
  if (isForeignKeyViolation(error)) {
    // Null = the count itself failed. Say "at least one" rather than inventing a zero: the delete has
    // already told us there IS a reference, and "appears on 0 booking lines and cannot be removed"
    // contradicts itself while hiding why the read failed.
    const count = await countRowsEq('booking_custom_items', 'rental_vehicle_slug', slug);
    const scale =
      count === null
        ? 'is still named on at least one booking line'
        : `appears on ${count} booking line${count === 1 ? '' : 's'}`;
    throw new Error(
      `This vehicle ${scale} and cannot be removed. Untick “Active” instead — it disappears from the rental page but stays readable on those bookings.`,
    );
  }
  throw error;
}
