  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      ...
      <div className="flex items-center justify-between px-[15px] pt-3.5">
        <div>
          <div className="font-display text-[18px] font-semibold text-ink">Your day</div>
          <div className="mt-px text-xs text-ink-muted">{stops.length ? `${stops.length} stops planned` : 'No stops yet'}</div>
        </div>
        <button
          type="button"
          onClick={onAddPlaces}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-[15px] py-3">
        {stops.length === 0 ? (
          <div className="px-2.5 py-[30px] text-center text-ink-muted">
            ...
            <p className="m-0 text-[13.5px] font-semibold text-[#7A8B8F]">Ask the co-pilot or add places</p>
            <p className="m-0 mt-1 text-[12.5px]">Stops appear here with live drive times.</p>
          </div>
        ) : (
          <div>
            {/* pickup → first */}
            <div className="flex items-center gap-[7px] pb-2 pl-7 text-[11.5px] font-semibold text-ink-muted">
              ...
              {segs[0]?.minutes ?? 0} min from pick-up
            </div>
            ...
            <div className="flex flex-col">
              {stops.map((p, i) => (
                <div key={p.id}>
                  ...
                  >
                    <span className="grid cursor-grab place-items-center px-0.5 py-1" aria-label="Drag to reorder">
