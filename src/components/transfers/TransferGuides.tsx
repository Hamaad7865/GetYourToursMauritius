...
const PREFERRED = [
  'mauritius-airport-transfer-guide',
  'getting-around-mauritius',
  'best-time-to-visit-mauritius',
];

/** "Mauritius Airport Transfer Tips & Guides" — links to our own travel guides. */
export async function TransferGuides({ count = 3 }: { count?: number }) {
  const t = await getT();
    ...
    <section className="mt-12 border-t border-ink/10 pt-9">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <h2 className="text-[22px] font-extrabold tracking-tight text-ink">
          {t('Mauritius airport transfer tips & guides')}
        </h2>
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {picked.map((p) => (
          ...
          >
            ...
            <h3 className="mt-2 text-[17px] font-extrabold leading-snug text-ink group-hover:text-teal">
              {p.title}
            </h3>
