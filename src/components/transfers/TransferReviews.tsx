...
export async function TransferReviews({ count = 6 }: { count?: number }) {
  const t = await getT();
        ...
        <div>
          ...
          <p className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[14px] text-ink/75">
            ...
            <span className="text-ink-muted">
              {t('from {total} reviews · TripAdvisor {ta} · Google {google}', {
                total: reviewStats.total.toLocaleString('en-GB'),
                ta: reviewStats.tripadvisor.rating.toFixed(1),
                google: reviewStats.google.rating.toFixed(1),
              })}
            </span>
          </p>
        </div>
