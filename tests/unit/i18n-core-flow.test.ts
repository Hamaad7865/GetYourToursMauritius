 ...
 *
 * Kept alongside the broad scanner in i18n-coverage.test.ts, which asserts that EVERY t() key in
 * the codebase has an entry. This file is narrower but stricter: it pins the money path
 * specifically, and asserts the French value is non-empty rather than merely present — a key
 * mapped to '' would satisfy the scanner and still render blank.
 */
