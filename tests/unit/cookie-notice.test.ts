...
describe('shouldShowNotice', () => {
  it('shows when nothing is stored', () => { expect(shouldShowNotice(null)).toBe(true); });
...
describe('serializeAck', () => {
  it('serializes the current version + the given timestamp', () => {
    expect(JSON.parse(serializeAck(123))).toEqual({ acknowledged: true, version: NOTICE_VERSION, ts: 123 });
  });
});
