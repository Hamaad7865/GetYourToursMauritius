...
describe('api_* service functions', () => {
  ...
  it('materialize_availability fills open-ended days; api_list_availability is a pure read', async () => {
    ...
    // We don't fulfil same-day bookings: the earliest bookable day is TOMORROW (Mauritius). So a query
    // anchored to today's Mauritius calendar day must return NOTHING, even before noon — the read
    // clamps its lower bound to tomorrow, matching create_hold's occurrence_too_soon guard.
