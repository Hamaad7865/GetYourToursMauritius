...
- **Auth:** public + optional bearer (`authenticateOptional`) — no token = anonymous; a *present-but-invalid*
...
type PlannerPlace = {
  ...
  closesAt: string | null; blurb: string | null; imageUrl: string | null; // imageUrl is a /api/planner/photo proxy URL
};
type PlannedRoute = {
  legs: { km: number; minutes: number }[];
  totalKm: number; totalMinutes: number; estimate: boolean; // estimate=true → haversine fallback
};
```
