...
| `SUPABASE_DB_PASSWORD`            | Supabase CLI (`link`, `db push`)   | The project's Postgres password — rotate it if it's ever been pasted anywhere else                                                                                                                                                                                                 |
| `SUPABASE_DB_URL`                 | direct `pg` queries (ledger reads) | **Use the SESSION POOLER URI**, Dashboard → Database → Connection string → Session pooler. NOT `db.<ref>.supabase.co` — that host is **IPv6-only** and GitHub Actions runners are IPv4-only, so the direct URI cannot connect from CI. Port 5432, not the 6543 transaction pooler. |
