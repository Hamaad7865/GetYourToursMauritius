...
| NEW `app/api/v1/admin/quotes/[ref]/balance/route.ts`            | staff-initiated balance link                                                                                                   |
...
## Task 5: Staff "send balance link" route + reconcile sweep

**Files:** new `app/api/v1/admin/quotes/[ref]/balance/route.ts`; thin client `sendBalanceLink` in `src/lib/admin/quotes.ts` (mirror `sendQuote` [:761-782]); `api_pending_payment_checkouts` branch; tests.

- [ ] Test FIRST: non-staff → 403 (copy the send route's `SENDING_ROLES={'admin','staff'}`, `'seo'` EXCLUDED, [send/route.ts:122]); a fully-paid booking → 409 (nothing owed); success returns `{url}` for the copy button; the balance email carries the token link and NEVER an internal note.

- [ ] Route order copies `admin/quotes/send/route.ts`: `rateLimit → requireUser → callerRole(profiles.role) → SENDING_ROLES → zod body → mint balance checkout via `createPaymentLink({purpose:'balance', authorizedBy:'quote', returnUrl})` with service-role ctx`. It points at the balance row's payment URL; it must NOT rotate `quotes.token_hash` (that would break the original quote link).

- [ ] `api_pending_payment_checkouts` ([20260910000000:942-948]) gains a `'balance'` branch swept by `pay.status in ('pending','failed')` (mirroring pickup_addon), so a lost balance webhook still reconciles. The DEPOSIT needs no new branch — it's a `booking` row on a `payment_pending` booking, already swept.

**Landmine:** the balance row must not let `run_booking_maintenance` expire the already-confirmed booking's seat. Confirm the 30-min payment_pending expiry keys on booking status, which is `confirmed` post-deposit — verify it can't touch the balance row.
